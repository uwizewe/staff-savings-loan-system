...
export default function LoanDashboardPage({ route }) {
  ...
  const origin=['applications','active'].includes(params.get('from'))?params.get('from'):'active';
  ...
  return <div className="min-w-0 space-y-6"><Button variant="ghost" onClick={()=>routeTo(`loans?view=${origin}`)}>← Back to {origin==='active'?'active loans':'loan applications'}</Button><PageHeader title="Loan dashboard" description={loan ? `${loan.applicationNumber} · ${loan.memberName}` : 'Loading loan'} actions={loan&&<Button variant="secondary" onClick={()=>setScheduleOpen(true)}>View Installment Schedule</Button>}/>
    {loading ? <Loading /> : error ? <ErrorState message={error} onRetry={()=>setRevision(r=>r+1)} /> : data && <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 min-[380px]:grid-cols-2 xl:grid-cols-4">{[['Original loan amount','originalPrincipal'],['Current principal','currentPrincipal'],['Total interest','totalInterest'],['Regular installment','monthlyInstallment'],['Total repayment','totalPayable'],['Principal paid','paidPrincipal'],['Interest paid','paidInterest'],['Total paid','amountRepaid'],['Remaining principal','remainingPrincipal'],['Outstanding balance','outstandingBalance']].map(([label,key])=><div key={key} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 font-bold"><MoneyCell value={loan[key]} /></p></div>)}{[['Installments paid',loan.installmentsPaid],['Installments remaining',loan.installmentsRemaining]].map(([label,value])=><div key={label} className="rounded-xl border border-teal-100 bg-teal-50 p-3"><p className="text-xs text-teal-700">{label}</p><p className="mt-1 font-bold">{value}</p></div>)}</div>
