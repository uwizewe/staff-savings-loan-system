...
export default function LoanCategoriesPage() {
  ...
  return <><PageHeader title="Loan Categories" description="System Settings · Configure loan rates and availability. Existing loans keep their agreed rate." actions={<Button onClick={()=>{setError('');setForm({...blank});}}>Add loan category</Button>} />
    {!form && error && <ErrorState message={error} />}
    {categories.loading?<Loading/>:categories.error?<ErrorState message={categories.error} onRetry={categories.reload}/>:<DataTable label="Loan categories" rows={categories.data} columns={[
