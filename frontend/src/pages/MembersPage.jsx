...
import { Button, DataTable, ErrorState, Field, inputClass, Loading, Modal, MoneyCell, PageHeader, SearchBox, StatusBadge, textareaClass, useNotice } from "../components/ui";
import { useApiData } from "../hooks";
import { date, routeTo, today } from "../utils";
    ...
    <>
      ...
      <Modal open={Boolean(editing)} onClose={close} title={editing === "new" ? "Register member" : "Update member"} description="Membership and risk status are managed separately." size="lg">
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          <Field label="Member ID" required><input className={inputClass} value={form.memberCode} onChange={(e) => setForm({ ...form, memberCode: e.target.value })} placeholder="STF-007" required /></Field>
