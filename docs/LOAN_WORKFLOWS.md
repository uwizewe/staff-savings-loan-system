...
Submission, approval and disbursement enforce one committed loan per staff member. A database reservation prevents concurrent pending, approved or active loans. Independent approval is required; disbursement cannot fall after the first installment date.

## Active loan dashboard and actions

Open **Loans → Active loans → View**. The dashboard shows staff details, loan terms, principal/interest/paid/balance summaries, installment progress, the current schedule, schedule versions and repayment history. Actions open separate dialogs without leaving Active Loans.
...
Select the month/year in **Loans → Monthly repayments**, enter the batch description, select due installments and enter payment amounts. Staff IDs/names, loan references, installment numbers, principal, interest and balances are available for review.
...
Member registration IDs are generated by the backend as `VFC-` followed by a UUID without hyphens. Existing IDs remain unchanged.
...
Validated: 22 backend tests, 5 frontend report tests, production frontend build and project structure checks. Coverage includes schedule version retention, reducing-balance calculations, category deactivation, first installment dates, one-term settlement, duplicate-payment prevention and monthly-batch rollback.
